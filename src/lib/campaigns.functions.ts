...
const campaignStatus = z.enum(["draft", "scheduled", "running", "paused", "completed", "failed"]);
...
export const createCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("run_logs").insert({
      ...
      user_id: context.userId,
      level: "info",
      message: `Campaign "${data.name}" queued with ${data.account_ids.length} accounts.`,
    });
...
export const listCampaigns = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("campaigns")
      .select(
        "id,name,type,status,scheduled_at,started_at,completed_at,posts_per_hour,total_targets,total_done,total_failed,created_at"
      )
...
export const listRuns = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: runs, error } = await context.supabase
      .from("campaign_runs")
      .select("id,status,started_at,completed_at,error,account_id,fb_accounts(label)")
